...
# Use OpenRouter Kilo-Auto model for all LLM operations
export OPENROUTER_MODEL="kilo-auto/free"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting research cycle (model: kilo-auto/free)..." >> "$LOG"
...
MINUTE=$(date '+%M')
if [ "$MINUTE" = "00" ]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Running HMM analysis (model: kilo-auto/free)..." >> "$LOG"
